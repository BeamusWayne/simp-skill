...
- **MBTI 分析系统**：新增 `/simp mbti` 指令。含三个模块：① 行为维度推断（E/I、N/S、T/F、J/P）② 16型专属追求策略（含核心需求、节奏、雷区、试探方式）③ 双方兼容性分析。信号解读与策略规划模块已集成 MBTI 校正层。
...
- **AI味检测**：`/simp message` 生成后自动运行，识别七种 AI 特征（过于工整的句式、缺少语气词、四字词堆砌等），命中后主动提示用户并给出改写方向，而不是替用户重写
- **危机扩展 C-11**：补充「对方开始了新感情后如何调整」——含72小时情绪处理、三问重新评估、路线A（长线等待）和路线B（拉开距离）分支，以及与 C-6（竞争对手）的区分说明

### v1.2 · 对话练习模式

目前 simp-skill 只生成话术，不帮用户练习怎么说。

v1.2 计划加入**对话练习模式** `/simp rehearse`：基于心上人档案构建一个简单的对方性格模拟，让用户在正式表白前「彩排」一遍。
...
- `/simp post`：**朋友圈内容建议**——今天发什么让ta注意到你，风格上怎么展示你最好的状态
- `/simp gift`：**礼物建议**——结合档案推荐有记忆点的礼物，而不是通用的节日礼物
- `/simp after`：**成功后模块**——在一起之后怎么维系热情、如何处理初期磨合，目前 v1 完全缺失这块

**体验改进类**

- **防耗尽检测**：检测用户短期内高频使用 + 情绪词负向趋势，如果明显焦虑化，主动说「先休息一下」
- **反向视角** `/simp mirror`：基于档案模拟「如果我是心上人，我对你的印象是什么」——帮用户跳出自己的视角
- **每周复盘** `/simp weekly`：本周高点/低点/下周策略，比 `/simp daily` 更长维度的回顾
